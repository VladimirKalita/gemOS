...
int is_valid_mem_range(unsigned long buff, u32 count, int access_bit)
{
        ...
        if(buff >= ctx->mms[MM_SEG_CODE].start && buff + count <= ctx->mms[MM_SEG_CODE].next_free-1){
                printk("yess\n");
                if(access_bit==1){
                        return 1;
                }
        ...
        else if(buff >= ctx->mms[MM_SEG_RODATA].start && buff + count <= ctx->mms[MM_SEG_RODATA].next_free-1){
                printk("yess1\n");
                if(access_bit==1)
{
                        return 1;
        ...
        else if(buff >= ctx->mms[MM_SEG_DATA].start && buff + count <= ctx->mms[MM_SEG_DATA].next_free-1){

                return 1;

        }

        else if(buff >= ctx->mms[MM_SEG_STACK].start && buff + count <= ctx->mms[MM_SEG_STACK].next_free-1){
...
long handle_ftrace_fault(struct user_regs *regs)
{
        ...
        while(temp){
                if(temp->faddr==faddr){
                        ...
                        //ret[narg+1]=-1;
                        int wr=strace_buffer_write(ctx->files[fd],(char *)ret,8*(1+narg));
                        char c[8]="breakit";
                        wr=strace_buffer_write(ctx->files[fd],c,8);
                }
                temp=temp->next;
        }
...
int sys_read_ftrace(struct file *filep, char *buff, u64 count)
{
        ...
        while(wrtn<count){
                while(1){
                        b=strace_buffer_read(filep,buff+rea,8);
                        if(strcmp(buff+rea,c)==0) break;
                        rea+=b;
                }
                wrtn++;
        }
